...
# including all socket.* and httplib.* exceptions.
    ...
    def request(self, uri, method="GET", body=None, headers=None, redirections=DEFAULT_MAX_REDIRECTS, connection_type=None, **kwargs):
        ...
        if error_type:
            from httplib2 import RelativeURIError, RedirectMissingLocation, ServerNotFoundError, FailedToDecompressContent
            from httplib2 import HttpLib2ErrorWithResponse, CertificateHostnameMismatch, CertificateValidationUnsupported
            from httplib2 import ProxiesUnavailableError, RedirectLimit, UnimplementedDigestAuthOptionError
            from httplib2 import MalformedHeader, SSLHandshakeError, NotSupportedOnThisPlatform, HttpLib2Error
            get_error = {
                "RelativeURIError": RelativeURIError(),
                "RedirectMissingLocation": RedirectMissingLocation(desc=1, response=2, content=3),
                "ServerNotFoundError": ServerNotFoundError(),
                "FailedToDecompressContent": FailedToDecompressContent(desc=1, response=2, content=3),
                "HttpLib2ErrorWithResponse": HttpLib2ErrorWithResponse(desc=1, response=2, content=3),
                "CertificateHostnameMismatch": CertificateHostnameMismatch(desc=1, host=2, cert=3),
                "CertificateValidationUnsupported": CertificateValidationUnsupported(),
                "ProxiesUnavailableError": ProxiesUnavailableError(),
                "RedirectLimit": RedirectLimit(desc=1, response=2, content=3),
                "UnimplementedDigestAuthOptionError": UnimplementedDigestAuthOptionError(desc=1, response=2, content=3),
                "UnimplementedHmacDigestAuthOptionError": UnimplementedHmacDigestAuthOptionError(desc=1, response=2, content=3),
                "MalformedHeader": MalformedHeader(),
                "SSLHandshakeError": SSLHandshakeError(),
                "NotSupportedOnThisPlatform": NotSupportedOnThisPlatform(),
